...
var MoveableData = /*#__PURE__*/function (_MoveableHelper) {
  _inherits(MoveableData, _MoveableHelper);

  var _super = _createSuper(MoveableData);

  function MoveableData(memory) {
    var _this;

    _classCallCheck(this, MoveableData);
    ...
    _defineProperty(_assertThisInitialized(_this), "selectedTargets", []);

    _this.memory = memory;
    return _this;
  }

  _createClass(MoveableData, [{
    ...
    value: function getSelectedFrames() {
      ...
      return this.selectedTargets.map(function (target) {
        return _this2.getFrame(target);
      });
    }
  }, {
    key: "renderFrames",
    value: function renderFrames() {
      ...
      this.selectedTargets.forEach(function (target) {
        _this3.render(target);
      });
    }
  }, {
    key: "setOrders",
    ...
    value: function setProperty(names, value) {
      return this.setValue(function (frame) {
        frame.set.apply(frame, _toConsumableArray(names).concat([value]));
      });
    }
  }, {
    ...
    value: function getProperties(properties, defaultValues) {
      ...
      return properties.map(function (property, i) {
        var frameValues = frames.map(function (frame) {
          return frame.get.apply(frame, _toConsumableArray(property));
        });
    ...
    value: function setValue(callback) {
      ...
      var infos = targets.map(function (target) {
        var frame = _this4.getFrame(target);
...
export { MoveableData as default };